...
const produto = [
    {id: '22', nome: 'Cerveja', quantidade: 500, valorunitario: 5, precototal: 0},
    {id: '26', nome: 'Danone', quantidade: 120, valorunitario: 4, precototal: 0} 

]
function calculo (prod){
    for(let i = 0; i < prod.length; i++){
        prod[i].precototal = prod[i].quantidade * prod[i].valorunitario
        prod[i].precovenda = prod[i].valorunitario + prod[i].valorunitario * 0.20
...
function retornar (request, response, next) {
    ...
    if(id === '' || nome === '' || quantidade === '' || valorunitario === '' || complemento === undefined){
...
server.put ("/produtos", retornar, (request, response) =>{
    ...
    if (filtrar.length === 0){
        return response.status(400).json({mensagem: 'Nao existe produto com este id'})
    }
...
server.delete ("/produtos" , (request, response) => {
    ...
    const filtrar = produto.find ((prod, index)=>{
        ...
        if(!filtrar){
            return response.status(400).json({mensagem: 'Nao existe produto com esse id'})
    }
    return response.json(filtrar)
})
